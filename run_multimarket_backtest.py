...
def load_a_share():
    """加载 A 股上证指数数据"""
    path = "data/raw/000001.SH_daily_2020-01-01_2024-01-01.csv"
    df = pd.read_csv(path)
...
def load_hk_share():
    """加载港股腾讯数据"""
    path = "data/raw/0700.HK_daily_2014-2025.csv"
    df = pd.read_csv(path)
    df['date'] = pd.to_datetime(df['date'])
    df = df.set_index('date').sort_index()
    df = df[['open', 'high', 'low', 'close', 'volume']].astype(float)
...
def load_us_share():
    ...
    """
    path = "data/raw/NVDA_daily_2014-2025.csv"
    ...
    df = df.sort_index()
    # 同时保存清洗后的文件供后续使用
    clean_path = "data/raw/NVDA_daily_2014-2025_clean.csv"
